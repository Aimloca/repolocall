...
class OrderProduct extends AppClass {
	constructor(params={}) {
		...
		if(this.row_id==undefined || this.row_id==null || this.row_id=='') this.row_id='_' + new Date().valueOf();
...
class Order extends AppClass {
	...
	constructor(params={}) {
		super(params);
		if(!this.id) { this.id='_' + document.cookie.match(/PHPSESSID=[^;]+/); this.id=this.id.replace('PHPSESSID=', ''); }
		var order_products=[];
	...
	MakeProductsLines() {
		...
		for(let product of this.products) {
			...
			if(product.paid==true || product.paid==1 || product.paid=='1') continue;
